...
console.log('search.js v3 загружен');
...
function addSearchPanel() {
    ...
    filterRow.innerHTML =
        '<input type="date" id="filterDateFrom" style="flex:1;min-width:110px;padding:8px;border:1px solid #E5E5EA;border-radius:8px;font-size:12px" onchange="applyFilters()">' +
        '<input type="date" id="filterDateTo" style="flex:1;min-width:110px;padding:8px;border:1px solid #E5E5EA;border-radius:8px;font-size:12px" onchange="applyFilters()">' +
        '<select id="filterCompany" style="flex:1;min-width:130px;padding:8px;border:1px solid #E5E5EA;border-radius:8px;font-size:12px" onchange="applyFilters()"><option value="">Все компании</option></select>' +
        '<button onclick="clearFilters()" style="padding:8px 12px;background:#FF3B30;color:#fff;border:none;border-radius:8px;font-size:12px;cursor:pointer">Сброс</button>';
...
// Перехватываем renderDevicesFromCache
var _origRender = renderDevicesFromCache;
...
var _origTab = switchDeviceTab;
switchDeviceTab = function(tab) {
    _origTab(tab);
    setTimeout(updateCompanies, 300);
    if (filtersActive) setTimeout(applyFilters, 500);
};

function applyFilters() {
    var query = (document.getElementById('deviceSearch') ? document.getElementById('deviceSearch').value : '').toLowerCase().trim();
    ...
    var company = document.getElementById('filterCompany') ? document.getElementById('filterCompany').value : '';
